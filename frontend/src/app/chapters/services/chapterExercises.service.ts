...
export class ChapterExercisesService {
  ...
  selectedAnswers: Record<number, number | null> = {};
    validateUnique(page: Page): boolean {
        if ( !page.Exercise || page.Exercise.type !== 'UNIQUE' ) {
            return false;
        }

        const selectedId = this.selectedAnswers[page.id_page];

        const selectedResponse = page.Exercise.UniqueResponses?.find((response: UniqueResponses) => response.id_response === selectedId);
  ...
  // ---------------- PAIRS
    ...
    pairsState: { [pageId: number]: { currentSelection: Pairs[], matchedIds: Set<number>, wrongIds: Set<number> } } = {};

    getPairsState(pageId: number): PairState {
        if (!this.pairsState[pageId]) {
            this.pairsState[pageId] = { currentSelection: [], matchedIds: new Set(), wrongIds: new Set() };
        }
  ...
  // ---------------- ORDER
    ...
    getOrderItems(page: Page): PutInOrders[] {
        ...
        if (!this.orderCache[pageId]) {
            this.orderCache[pageId] = [ ...(page.Exercise?.PutInOrders ?? []) ].sort((a, b) => a.mixed_order - b.mixed_order);
        }
        return this.orderCache[pageId];
    }

    moveOrderItem( page: Page, previousIndex: number, currentIndex: number ) {
    ...
    validateOrder(page: Page): boolean {
        ...
        const correct = [...items].sort((a, b) => a.correct_order - b.correct_order).map(i => i.id_response);
